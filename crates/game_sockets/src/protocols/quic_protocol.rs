                        ...
                        match cmd {
                            ...
                            BackendCommand::Connect { addr, port } => {
                                ...
                                match endpoint.connect(remote, "localhost").unwrap().await {
                                    ...
                                    Err(e) => {
                                        error!("QUIC Connection Timeout/Failed: {:?}", e);
                                        // On renvoie une erreur propre au thread principal sans crasher
                                        let _ = event_tx.send(GameNetworkEvent::Error {
                                            connection: Default::default(), // UUID vide
...
impl QuicBackend {
    ...
    fn spawn_reader(
        ...
        let conn_death_watcher = conn.clone();
        let event_tx_death = event_tx.clone();
        ...
        tokio::spawn(async move {
            // Wait for the end of quik connection
            let _reason = conn_death_watcher.closed().await;

            // On prévient l'ECS immédiatement
            let _ = event_tx_death.send(GameNetworkEvent::Disconnected(uuid.into()));
        });
    }
