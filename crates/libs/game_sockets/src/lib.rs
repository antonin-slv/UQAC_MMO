...
impl GamePeer {
    ...
    pub fn close_stream(&self, conn: GameConnection, stream: GameStream) -> Result<(), GameSocketError> {
        self.send_cmd(BackendCommand::CloseStream { connection: conn.connection_uuid, stream: stream.stream_id })
    }

    pub fn send(&self, conn: &GameConnection, stream: &GameStream, msg: Bytes) -> Result<(), GameSocketError> {
        self.send_cmd(BackendCommand::Send { connection: conn.connection_uuid, stream: stream.clone(), data: msg })
    }
